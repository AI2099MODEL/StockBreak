...
object WorkerUtils {
    ...
    fun scheduleBreakoutPerformanceWorker(context: Context) {
        val constraints = Constraints.Builder()
            .setRequiredNetworkType(NetworkType.CONNECTED)
            .build()

        val workRequest = PeriodicWorkRequestBuilder<BreakoutPerformanceWorker>(15, TimeUnit.MINUTES)
            .setConstraints(constraints)
            .build()

        WorkManager.getInstance(context).enqueueUniquePeriodicWork(
            "BreakoutPerformanceWorker",
            ExistingPeriodicWorkPolicy.KEEP,
            workRequest
        )
    }
